...
def get_merval():
    """
    Fuente primaria: open.bymadata.com.ar (sin auth, 20min delay)
    Fuente secundaria: estadisticasbcra.com (último valor diario)
    Fuente terciaria: Yahoo Finance (fallback con retry)
    """
    # --- Fuente 1: Open BYMA Data (público, sin key) ---
    try:
        headers = {
        ...
        with httpx.Client(timeout=10) as client:
            r = client.get(
                "https://open.bymadata.com.ar/vanoms-be-core/rest/api/bymadata/free/index-portfolio-assets/index-evolution",
                params={"index": "MERVAL"},
                headers=headers
            )
        if r.status_code == 200:
            ...
            if data and len(data) >= 2:
                ultimo = data[-1].get("closePrice", 0)
                anterior = data[-2].get("closePrice", 0)
                if ultimo and anterior:
                    change = ((ultimo - anterior) / anterior) * 100
                    return {"price": ultimo, "change": change, "fuente": "BYMA"}
    except Exception as e:
        logger.warning(f"BYMA Merval falló: {e}")

    # --- Fuente 2: estadisticasbcra.com (serie histórica Merval) ---
    try:
        with httpx.Client(timeout=10) as client:
            r = client.get(
                "https://api.estadisticasbcra.com/merval",
                headers={"Authorization": "BEARER"}
            )
        if r.status_code == 200:
            data = r.json()
            if data and len(data) >= 2:
                ultimo = float(data[-1].get("v", 0))
                anterior = float(data[-2].get("v", 0))
                if ultimo and anterior:
                    change = ((ultimo - anterior) / anterior) * 100
                    return {"price": ultimo, "change": change, "fuente": "BCRA Stats"}
    except Exception as e:
        logger.warning(f"BCRA Stats Merval falló: {e}")

    # --- Fuente 3: Yahoo Finance (fallback con retry y backoff) ---
    for intento in range(3):
        try:
            time.sleep(intento * 2)  # backoff: 0s, 2s, 4s
            with httpx.Client(timeout=15) as client:
                r = client.get(
                    "https://query2.finance.yahoo.com/v8/finance/chart/%5EMERV",
            ...
            if r.status_code == 200:
                ...
                return {"price": price, "change": change, "fuente": "Yahoo"}
        except Exception as e:
            logger.warning(f"Yahoo Merval intento {intento+1} falló: {e}")
...
def get_cedears():
    ...
    try:
        with httpx.Client(timeout=12, headers=headers) as client:
            for t in tickers:
                try:
                    time.sleep(0.8)  # evitar 429
                    r = client.get(f"https://query2.finance.yahoo.com/v8/finance/chart/{t}")
                    data = r.json()
